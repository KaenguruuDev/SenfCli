...
public class CommandHandlers
{
	...
	public static async Task Reconcile()
	{
		try
		{
			...
			var localContent = File.Exists(project.EnvPath) ? await File.ReadAllTextAsync(project.EnvPath) : string.Empty;
			...
			if (remote == null)
			{
				...
				if (resp == "y" || resp == "yes")
				{
					...
					if (profile.SshKeyId > -1)
						Config.SaveFileHash(project.ProjectName!, profile.SshKeyId, Config.ComputeStringHash(localContent));
					return;
				}
